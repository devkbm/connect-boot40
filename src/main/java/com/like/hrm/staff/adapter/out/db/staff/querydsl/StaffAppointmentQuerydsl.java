	...
	public List<StaffAppointmentQueryResultDTO> query(StaffAppointmentQueryDTO dto) {
		...
		return queryFactory
				.select(Projections.fields(StaffAppointmentQueryResultDTO.class, 
						...
						qRecord.info.dutyResponsibilityCodeList.as("dutyResponsibilityCode"),
						qRecord.info.dutyResponsibilityCodeList.as("dutyResponsibilityName")
				...
				.where(
						...
						eqAppointmentTypeCode(dto.appointmentTypeCode()),
						betweenDate(dto.from(), dto.to())
				)
				.fetch();
	}
	
	private BooleanExpression eqCompanyCode(String companyCode) {
		return hasText(companyCode) ? qStaff.id.companyCode.eq(companyCode) : null;					
	}
	
	private BooleanExpression eqStaffNo(String staffNo) {
		return hasText(staffNo) ? qStaff.id.staffNo.eq(staffNo) : null;					
	}
	
	private BooleanExpression eqAppointmentTypeCode(String appointmentTypeCode) {
		return hasText(appointmentTypeCode) ? qRecord.appointmentTypeCode.eq(appointmentTypeCode) : null;					
	}
	
	private BooleanExpression betweenDate(LocalDate from, LocalDate to) {
		return from != null && to != null ? qRecord.appointmentDate.between(from, to) : null;				
	}
